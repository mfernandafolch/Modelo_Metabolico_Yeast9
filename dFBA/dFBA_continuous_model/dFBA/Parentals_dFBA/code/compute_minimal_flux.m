function [GeneClasses RxnClasses modelIrrevFM MinimizedFlux match2rev] = compute_minimal_flux(model, varargin)
...
if mod(length(varargin),2)==0
    for i=1:2:length(varargin)-1
        switch lower(varargin{i})
            ...
            case 'inhibitor', Inhibitor = varargin{i+1};
...
%     model.lb(model.c~=0) = FBAsoln.v(model.c~=0);
%     model.ub(model.c~=0) = FBAsoln.v(model.c~=0);

    model.lb(model.c~=0) = FBAsoln.v(model.c~=0)-abs(FBAsoln.v(model.c~=0).*1e-4);
    model.ub(model.c~=0) = FBAsoln.v(model.c~=0)+abs(FBAsoln.v(model.c~=0).*1e-3);
        
    [ MinimizedFlux modelIrrevFM match2rev]= minimizeModelFlux_local(model,GeneOption);
%     basisPFBA=MinimizedFlux.basis;
...
function [ MinimizedFlux modelIrrev match2rev]= minimizeModelFlux_local(model,GeneOption)
...
% minimized flux and an irreversible model convert model to irrev
    ...
    [modelIrrev match2rev] = convertToIrreversible(model);
